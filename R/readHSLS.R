...
readHSLS <- function(path = getwd(),
  ...
  retryProc <- tryCatch({processedData <- do.call("processHSLS_SPSS", processArgs, quote = TRUE)
                                                    FALSE
                                                  }, error = function(e){
                                                    TRUE #flag to retry
                                                  }, warning = function(w){
  ...
  if (retryProc){
    ...
    processedData <- tryCatch(do.call("processHSLS_SPSS", processArgs, quote = TRUE),
                              error = function(e){
                                stop(paste0("Unable to process HSLS data. Possible file corruption with source data.",
                                            " Error message: ", e))
                              })
  }
  ...
  omittedLevels <- c("Data suppressed", "DATA SUPPRESSED",
...
buildHSLSWeightList <- function(fileFormat1, fileFormat2){
  ...
  for(i in 1:length(wgtVars)){
    ...
    if(!is.null(fileFormat2)){
      ujkz <- unique(c(tolower(grep(paste0("^","(", tempVar ,")","[0-9]"), fileFormat1$variableName, value = TRUE, ignore.case = TRUE)),
