...
mvrlm.sdf <- function(formula,
  ...
  if (inherits(data, c("edsurvey.data.frame.list"))) {
    ...
    return(calc.mvrlm.sdf(formula=formula,
                          data=data,
                          weightVar=weightVar,
                          relevels=relevels,
                          jrrIMax=jrrIMax,
...
calc.mvrlm.sdf <- function(formula,
  ...
  coefmatlist <- list()
  for(i in 1:nDV){
    coefmatlist[[length(coefmatlist) + 1]] <- data.frame(coef = coef[,i],
...
coef.edsurveyMvrlm <- function(object, ...) {
  ...
  names(coef) <- apply(X = names, MARGIN = 1, FUN = function(x){paste(x, collapse = "_")})
  coef
}
...
coef.edsurveyMvrlmList <- function(object, ...) {
  ...
  for(i in 1:object[[1]]$nDV){
    cat(paste0("\n", yvar[i], "\n"))
    print(sapply(object, function(x){x$coef[,i]}))
    cat("\n")
    coeflist[[yvar[i]]] <- sapply(object, function(x){x$coef[,i]})
  }
  return(coeflist)
}
...
makeHypothesis.edsurveyMvrlm <- function(model, hypothesis){
  ...
  varnames <- apply(X = names, MARGIN = 1, FUN = function(x){paste(x, collapse = "_")})
