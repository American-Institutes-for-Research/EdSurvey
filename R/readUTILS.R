...
#takes a tibble object (spssDF) after it has been read in from a haven 'read' call and returns a fileFormat object of the tibble describing it's properties
...
writeDF_FWF <- function(df, fileFormat, savePath, verbose){
  ...
  outFile <- tryCatch({file(savePath, "w")},
           error = function(e){
...
parseSAS_FileFormat_HSB <- function(sasFile){
  ...
  ordPos <- sapply(inputLinesSplit, function(x){x[1]})
  ...
  inputLinesSplit <- sapply(inputLinesSplit, function(x){x[2]})
  ...
  fwfPos <- sapply(inputLinesSplit, function(x){x[2]})
  fwfPos <- strsplit(fwfPos, "-", fixed = TRUE)
  
  startPos <- as.numeric(trimws(sapply(fwfPos, function(x){x[1]}), which = "both"))
  endPos <- as.numeric(trimws(sapply(fwfPos, function(x){x[2]}), which = "both"))
  ...
  varInfo <- trimws(sapply(inputLinesSplit, function(x){x[1]}), which = "both")
  varInfoSplit <- strsplit(varInfo, " ", fixed = TRUE)
  varInfoSplit <- lapply(varInfoSplit, function(x){x[x!=""]}) #remove any blank/empty spacing elements from strsplit
  
  varName <- tolower(sapply(varInfoSplit, function(x){x[2]}))
  dataType <- sapply(varInfoSplit, function(x){x[3]})
  
  
  #grab the numeric precision and scale
  precision <- suppressWarnings(sapply(strsplit(dataType,".", fixed = TRUE), function(x){as.numeric(tail(x,1))}))
  scale <- suppressWarnings(sapply(strsplit(dataType,".", fixed = TRUE), function(x){as.numeric(head(x,1))}))
  ...
  lblVar <- tolower(sapply(lblSplit, function(x){x[1]}))
  lblDesc <- sapply(lblSplit, function(x){paste(x[-1], sep="", collapse = "=")}) #in case there is a '=' symbol use paste here
  ...
  for(testLine in lblLines){
    ...
    if(grepl("^VALUE ", testLine, ignore.case = TRUE)){
      ...
      val <- sapply(splitVals, function(x){x[1]})
      desc <- sapply(splitVals, function(x){paste(x[-1], sep="", collapse = "=")})
